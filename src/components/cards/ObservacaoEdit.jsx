  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-overlay)' }} onClick={onClose}>
      <div style={{ background: 'var(--bg-surface1)', border: '1px solid var(--border-default)', borderRadius: 16, padding: 24, width: 384, boxShadow: 'var(--shadow-modal)', animation: 'scaleIn 0.18s ease' }} onClick={e => e.stopPropagation()}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: 11, fontFamily: 'DM Mono, monospace', margin: '0 0 16px' }}>{card.title}</p>

        <textarea
          value={text} onChange={e => setText(e.target.value)} rows={5}
          placeholder="Observação..." autoFocus
          style={{ width: '100%', background: 'var(--bg-input)', border: '1px solid var(--border-default)', borderRadius: 10, padding: '12px 14px', color: 'var(--text-primary)', fontSize: 13, outline: 'none', resize: 'none', fontFamily: 'Syne, sans-serif', transition: 'border-color 0.13s', boxSizing: 'border-box' }}
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
          ...
          <button onClick={handleSave} disabled={loading}
            style={{ flex: 1, padding: '10px', borderRadius: 10, border: 'none', background: 'var(--accent-blue)', color: '#fff', fontSize: 13, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.6 : 1, transition: 'background 0.13s' }}
